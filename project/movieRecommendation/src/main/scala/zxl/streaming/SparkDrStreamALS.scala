package zxl.streaming

import java.sql.Connection

import zxl.caseclass.{Result, UserRating}
import zxl.utils.{DBLocalUtils, DBUtils}
...
//          println(1)
          ...
          val movieIds =List.apply(uidDFArray.map(_.split(",")).filter(x => x(2).trim().toDouble >= 4.0).map(x => x(1).trim().toInt))
