...
object ETL {
  def main(args: Array[String]) {
    ...
    val links = sc.textFile("src/main/resources/data/links.txt", minPartitions) //d
      .filter { !_.endsWith(",") } //e
      .map(_.split(",")) //e
      .map(x => Links(x(0).trim.toInt, x(1).trim.toInt, x(2).trim().toInt)) //e
      .toDF()

    val movies = sc.textFile("src/main/resources/data/movies.txt", minPartitions).filter { !_.endsWith(",") }
    ...
    val tags = sc.textFile("src/main/resources/data/tags.txt", minPartitions).filter { !_.endsWith(",") }.map(x=>rebuild(x)).map(_.split(",")).map(x => Tags(x(0).trim().toInt, x(1).trim().toInt, x(2).trim(), x(3).trim().toInt)).toDF()
